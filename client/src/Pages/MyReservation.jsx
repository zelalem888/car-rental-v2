  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const confirmedList = allData.filter((item)=> item.Status == "confirmed")

        setPending(pendingList)
        setConfirmed(confirmedList)

          const historyResponse = await fetch(`http://localhost:3000/api/history/${id}`);
      if (!historyResponse.ok) {
        console.log("there is no rent.");
      }

      const historyResult = await historyResponse.json()
      const filteredHIstory = historyResult.filter((item)=> item.Refund != null)
              ...
              <div className="w-2/3 p-5 flex flex-col gap-3">
                ...
                <div className="mt-3 grid grid-cols-2 border-t pt-3 text-sm">
                  ...
                  <p>
                   ...
                   <p>
                    <strong>Rented Days:</strong>{" "}
                    {item.Total_Rent_Day} Day/s
                  </p>
                  <p>
                    <strong>Total Payment:</strong>{" "}
                    {item.Total_paid} Birr
                  </p>
                  <p> 
                    <strong>Over payment:</strong>{" "}
                    {item.over_payment || 0} birr
                  </p>
                  <p> 
