...
const ConfirmedReservations = () => {
  const {id} = useParams()
    ...
    useEffect(() => {
      const fetchData = async () => {
        const response = await fetch("http://localhost:3000/api/reservation/vehicle/confirmed");
        if (!response.ok) {
  ...
  const moreDetail = async ({C_ID ,  V_ID})=>{
   ...
   const customerResponse = await fetch(`http://localhost:3000/api/user/admin/${C_ID}`);
    ...
    return (
      <div className="p-8 bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">confirmed Vehicle List</h2>
